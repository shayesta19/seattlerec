...
A white Space Needle with a Galaxy Gold top house — the 1962 World's Fair
scheme, where the legs went up in Astronaut White and only the top house was
gold, and the colour the halo was repainted for the 50th in 2012. A Needle
that is orange all over is not one anybody has actually seen.

```bash
npm run icons
```

Everything in `public/` with `icon` or `favicon` in the name is generated by
that, from one copy of the artwork in `tools/icons.mjs`. The set needs three
different framings of the same drawing and they have to agree: browsers get a
rounded badge, iOS gets square corners because it applies its own mask (round
it here and it gets rounded twice), and the maskable icon keeps its artwork
inside the 80% safe zone or Android crops the spire and the feet off. Five
files that must not drift is how they drift.

Two things worth knowing if you redraw it. The observation deck really sits at
86% of the tower's height, but drawn at 16px that is a vertical line with a
speck on top, so the proportions here are stylised — what had to stay true is
their *order*: saucer high, long waist, narrow feet. And a wide flat bar
halfway up with the feet apart does not read as a tower, it reads as a person
with their arms out; the saucer is an ellipse with a concave underside for
exactly that reason.
...
| `tools/icons.mjs` | The favicon set — artwork, every size, and the manifest. |
